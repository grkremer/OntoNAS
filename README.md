...
| **Data** | Object Kind | An informational entity consumable by computational procedures during a search run. |
...
| **Hyperparameters** | Quality Kind | A configuration entity representing the configurable, non-learned attributes of a model, it specializes into *Neural Network* and *Layer* hyperparameters. |
...
| **NAS Experiment** | Event | A perdurant representing the temporally bounded execution of a NAS run, triggered by the need for a trained network and resulting in a finished-experiment situation, it aggregates the participations of its substantials and in its course *Candidate* and *Final Architectures* come to exist. |
...
| **6** | How many cores and what operating frequency did the hardware have? | `core and isComponentOf some (hardware_device and participatedIn some nas_experiment)operating_frequency and inheresIn some (hardware_device and participatedIn some nas_experiment)` |
